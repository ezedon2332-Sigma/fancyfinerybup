...
type Status = "idle" | "checking" | "sending" | "sent" | "reset-sent";
 ...
 *
 * The customer enters an email first; a rate-limited server check decides
 * whether they already have an account. Existing → straight to a "welcome back"
 * password step. New → the create-account form. Nobody who already has an
 * account is shown a signup form, and a returning customer is greeted by name.
 */
export function LoginForm() {
  ...
  /** Step 1 → decide the path from whether the email has an account. */
  async function handleContinue(e: React.FormEvent) {
    ...
    setStatus("checking");
    setMessage(null);
    const res = await checkEmailExists(parsed.data.email);
    setStatus("idle");
    // Existing OR uncertain → sign-in step (with a create-account escape hatch);
    // only a confident "no account" sends them to registration.
    setStep(res.ok && !res.exists ? "signup" : "password");
  }
  ...
  async function handlePassword(e: React.FormEvent) {
    ...
    // Greet, then hand off with a full navigation so Server Components re-read
      ...
      <AnimatePresence mode="wait" initial={false}>
        ...
        {status === "sent" || status === "reset-sent" ? (
          ...
          >
            ...
            <h1 className="brand-wordmark mt-5 text-2xl">Welcome back</h1>
            <p className="mt-2 break-words text-sm text-gray-300">{email}</p>
          ...
          >
            ...
            <p className="mt-1.5 text-sm text-gray-400">
              You&apos;re new here — let&apos;s set you up.
            </p>
            <div className="mt-6">
          ...
          /* -------- Returning customer: welcome-back sign in -------- */
          <motion.div
            key="password"
            initial={{ opacity: 0, x: 24 }}
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            ...
            <p className="mt-1.5 break-words text-sm text-gray-400">{email}</p>

            <form
              onSubmit={mode === "password" ? handlePassword : (e) => { e.preventDefault(); handleMagicLink(); }}
            ...
            <p className="mt-4 text-center text-xs text-gray-500">
              ...
              <button
                ...
                className="font-medium text-yellow-400 underline underline-offset-4 hover:text-yellow-300"
              >
                Create an account
              </button>
            </p>
          ...
          >
            <div className="text-center">
              <h1 className="text-2xl font-bold text-yellow-400">
                {signupIntent ? "Create your account" : "Welcome"}
              </h1>
              <p className="mt-2 text-sm text-gray-400">
                {signupIntent
                  ? "Enter your email to begin — we'll sign you in if you already have an account."
