...
export default async function RootLayout({
  ...
  // Origin only — preconnect takes a scheme and host and ignores any path.
  const mediaOrigin = new URL(publicEnv.mediaUrl).origin;
  ...
  return (
    ...
    >
      <head>
        {/*
          Product media is served from its own origin, so the first image on
          a page otherwise pays a cold DNS + TCP + TLS handshake before a
          byte of it moves — three round trips, which on a mobile connection
          far from the server is most of a second of blank card.

          preconnect opens that connection while the HTML is still parsing,
          so the handshake overlaps work the browser is doing anyway.
          Derived from NEXT_PUBLIC_MEDIA_URL rather than hardcoded, so it
          cannot drift from the host the images actually come from.
        */}
