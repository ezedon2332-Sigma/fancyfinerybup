...
export async function searchCatalog(input: {
  ...
  // Filters (.eq/.or/.lte) first, then transforms (.order/.limit).
  let filter = admin
    .from("products")
    .select(
      "name, slug, price, product_images(storage_path, media_type, sort_order), product_variants(size, color, stock_qty)",
    )
    .eq("status", "published");

  if (q.length >= 2) {
    filter = filter.or(`name.ilike.%${q}%,description.ilike.%${q}%`);
  }
  const maxNaira = Number(input.max_price_naira);
  if (Number.isFinite(maxNaira) && maxNaira > 0) {
    filter = filter.lte("price", Math.round(maxNaira * 100));
  }
  ...
  const cards: ConciergeProduct[] = data.map((row) => {
    ...
    const primary =
      [...images].sort((a, b) => a.sort_order - b.sort_order).find(
    ...
    const inStock =
      variants.length === 0 ? true : variants.some((v) => v.stock_qty > 0);
    return {
      name: row.name,
      slug: row.slug,
      price: formatMoney(row.price, "NGN"),
      imageUrl: primary ? resolveImageUrl(primary.storage_path) : null,
      sizes,
      colors,
      inStock,
    };
